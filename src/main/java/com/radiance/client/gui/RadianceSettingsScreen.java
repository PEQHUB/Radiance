...
public class RadianceSettingsScreen extends GameOptionsScreen {
    ...
    protected void addOptions() {
        ...
        // Bloom toggle
        SimpleOption<Boolean> bloomEnabled = SimpleOption.ofBoolean(
            Options.BLOOM_ENABLED_KEY, Options.bloomEnabled,
            value -> Options.setBloomEnabled(value, true));
        this.body.addSingleOptionEntry(bloomEnabled);

        if (Options.bloomEnabled) {
            // Bloom Threshold: 0.1–5.0 (stored as tenths 1–50, default 10 = 1.0)
            ResettableSliderWidget bloomThreshSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                1, 50, Options.bloomThresholdTenths, 10,
                v -> getGenericValueText(
                    Text.translatable(Options.BLOOM_THRESHOLD_KEY),
                    Text.literal(String.format("%.1f", v / 10.0))),
                v -> Options.setBloomThreshold(v, true));
            this.body.addEntry(new SliderEntry(bloomThreshSlider, body));

            // Bloom Strength: 0.00–2.00 (stored as tenths 0–200, default 1 = 0.08 ~ we store raw)
            ResettableSliderWidget bloomStrengthSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                0, 200, Options.bloomStrengthTenths, 1,
                v -> getGenericValueText(
                    Text.translatable(Options.BLOOM_STRENGTH_KEY),
                    Text.literal(String.format("%.2f", v / 100.0))),
                v -> Options.setBloomStrength(v, true));
            this.body.addEntry(new SliderEntry(bloomStrengthSlider, body));

            // Bloom Radius: 1–16 pixels
            ResettableSliderWidget bloomRadiusSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                1, 16, Options.bloomRadius, 4,
                v -> getGenericValueText(
                    Text.translatable(Options.BLOOM_RADIUS_KEY),
                    Text.literal(v + " px")),
                v -> Options.setBloomRadius(v, true));
            this.body.addEntry(new SliderEntry(bloomRadiusSlider, body));
        }

        // Motion Blur toggle
        SimpleOption<Boolean> motionBlurEnabled = SimpleOption.ofBoolean(
            Options.MOTION_BLUR_ENABLED_KEY, Options.motionBlurEnabled,
            value -> Options.setMotionBlurEnabled(value, true));
        this.body.addSingleOptionEntry(motionBlurEnabled);

        if (Options.motionBlurEnabled) {
            // Motion Blur Strength: 0.1–2.0 (stored as tenths 1–20, default 10 = 1.0)
            ResettableSliderWidget mbStrengthSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                1, 20, Options.motionBlurStrengthTenths, 10,
                v -> getGenericValueText(
                    Text.translatable(Options.MOTION_BLUR_STRENGTH_KEY),
                    Text.literal(String.format("%.1f", v / 10.0))),
                v -> Options.setMotionBlurStrength(v, true));
            this.body.addEntry(new SliderEntry(mbStrengthSlider, body));

            // Motion Blur Samples: 4–16 (default 8)
            ResettableSliderWidget mbSamplesSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                4, 16, Options.motionBlurSamples, 8,
                v -> getGenericValueText(
                    Text.translatable(Options.MOTION_BLUR_SAMPLES_KEY),
                    Text.literal(String.valueOf(v))),
                v -> Options.setMotionBlurSamples(v, true));
            this.body.addEntry(new SliderEntry(mbSamplesSlider, body));
        }

        // Depth of Field toggle
        SimpleOption<Boolean> dofEnabled = SimpleOption.ofBoolean(
            Options.DOF_ENABLED_KEY, Options.dofEnabled,
            value -> {
                Options.setDofEnabled(value, true);
                MinecraftClient.getInstance().setScreen(new RadianceSettingsScreen(parentScreen));
            });
        this.body.addSingleOptionEntry(dofEnabled);

        if (Options.dofEnabled) {
            // DoF Auto-Focus toggle
            SimpleOption<Boolean> dofAutoFocus = SimpleOption.ofBoolean(
                Options.DOF_AUTO_FOCUS_KEY, Options.dofAutoFocus,
                value -> {
                    Options.setDofAutoFocus(value, true);
                    MinecraftClient.getInstance().setScreen(new RadianceSettingsScreen(parentScreen));
                });
            this.body.addSingleOptionEntry(dofAutoFocus);

            if (!Options.dofAutoFocus) {
                // Focal Distance: 1.0–50.0 m (stored as decimetres 10–500, default 160 = 16.0)
                ResettableSliderWidget focalSlider = new ResettableSliderWidget(
                    0, 0, 150, 20,
                    10, 500, Options.dofFocalDistanceDm, 160,
                    v -> getGenericValueText(
                        Text.translatable(Options.DOF_FOCAL_DISTANCE_KEY),
                        Text.literal(String.format("%.1f m", v / 10.0))),
                    v -> Options.setDofFocalDistance(v, true));
                this.body.addEntry(new SliderEntry(focalSlider, body));
            }

            // Aperture: 0.01–0.50 (stored as hundredths 1–50, default 5 = 0.05)
            ResettableSliderWidget apertureSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                1, 50, Options.dofApertureHundredths, 5,
                v -> getGenericValueText(
                    Text.translatable(Options.DOF_APERTURE_KEY),
                    Text.literal(String.format("%.2f", v / 100.0))),
                v -> Options.setDofAperture(v, true));
            this.body.addEntry(new SliderEntry(apertureSlider, body));

            // Max Radius: 1–32 pixels (default 12)
            ResettableSliderWidget maxRadiusSlider = new ResettableSliderWidget(
                0, 0, 150, 20,
                1, 32, Options.dofMaxRadius, 12,
                v -> getGenericValueText(
                    Text.translatable(Options.DOF_MAX_RADIUS_KEY),
                    Text.literal(v + " px")),
                v -> Options.setDofMaxRadius(v, true));
            this.body.addEntry(new SliderEntry(maxRadiusSlider, body));
        }
