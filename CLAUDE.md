...
Current baseline from `gradle.properties`: **MC 1.20.1 / Yarn 1.20.1+build.10 / Fabric Loader 0.15.11 / Fabric API 0.92.6 / Java 17 / Loom 1.11-SNAPSHOT**.
...
JUnit tests live under `src/test/java` and run with `./gradlew test`. The native renderer is built separately via CMake in the MCVR repo (see README "Build" section and `.github/workflows/build-linux.yml` for the exact commands and required deps — Vulkan SDK + glslang, plus several X11/Wayland libs on Linux).
...
- Windows: `natives/windows/core.dll`; `core.lib` is optional/runtime-unneeded unless MCVR proves otherwise
...
- `com.radiance.client.RadianceClient` — `ClientModInitializer`. This is where the real bootstrap happens: extracts `core.dll`/`libcore.so` (and optional Streamline DLLs) from the jar to `.minecraft/radiance/`, `System.load`s the native lib, runs the ABI handshake, copies bundled `shaders/` and `modules/` next to it, then initializes native-backed services. Missing native/resources or handshake failure disables Radiance and leaves vanilla boot alive.

### Three layers

1. **Mixins (`com.radiance.mixins.*`)** — declared in `src/main/resources/radiance.mixins.json`, gated by the `MixinPlugin.ENABLED_MIXINS` allowlist. Two top-level packages:
...
- The "O" keybinding is registered in `KeyInputHandler`; the settings screen is currently deferred/no-op during the 1.20.1 backport. The welcome message in chat is shown once per successful native-backed initialization when `Options.showWelcomeMessage` is true.
